...
public abstract class BaseAgent {
    ...
    protected void sendMessage(AgentMessage message) {
        if (currentSseEmitter != null) {
            try {
                currentSseEmitter.send(JSONUtil.toJsonStr(message));
            } catch (IOException e) {
                log.error("发送消息失败", e);
            }
        }
        ...
        CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                for (int i = 0; i < maxSteps && state != AgentState.FINISHED; i++) {
                    ...
                    log.info("Executing step {}/{}", stepNumber, maxSteps);
                    // 发送步骤信息
                    sendMessage(AgentMessage.stepInfo(stepNumber, maxSteps));
                    // 单步执行
                    String stepResult = step();
                    String result = "Step " + stepNumber + ": " + stepResult;
                    results.add(result);
                ...
                if (currentStep >= maxSteps) {
                    state = AgentState.FINISHED;
                    results.add("Terminated: Reached max steps (" + maxSteps + ")");
                    sendMessage(AgentMessage.error("执行结束：达到最大步骤（" + maxSteps + "）"));
                }
                // 正常完成
                sseEmitter.complete();
            } catch (Exception e) {
                ...
                log.error("error executing agent", e);
                try {
                    sendMessage(AgentMessage.error("执行错误：" + e.getMessage()));
                    sseEmitter.complete();
                } catch (Exception ex) {
